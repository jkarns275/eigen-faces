...
face2=[];
for i=1:4
    [eigenfaces_1,k_1,construct_1,y_1,t_1]=dimention_reduction(data_400,@pca_svd,0,th(i),1);
    [eigenfaces_2,k_2,construct_2,y_2,t_2]=dimention_reduction(data_400,@pca_eigen,0,th(i),1);
    k_variance(1,i)=k_1;
    k_variance(2,i)=k_2;
    t(1,i)=t_1;
    t(2,i)=t_2;
...
k=130;
[eigenfaces_f,k_f,construct_f,y_f]=dimention_reduction(flower_face,@pca_svd,k,0,0);
acc_rate_f=classification(y_f,size(flower_face_train,2), ff_train_label, ff_test_label);

    %% face identification
k=90;
[eigenfaces,k_c,construct,y]=dimention_reduction(input_data,@pca_svd,k,0,0);
acc_rate_c=classification(y,size(train_data,2), train_label, test_label);
